...
mod tests {
    ...
    #[test]
    fn opening_a_file_invalidates_old_completion_results() {
        let _guard = crate::TEST_ENV_LOCK.lock().unwrap();
        let old_config_dir = std::env::var_os("AITEXT_CONFIG_DIR");
        let config_dir =
            std::env::temp_dir().join(format!("aitext-open-test-{}", std::process::id()));
        let path = config_dir.join("opened.rs");
        std::fs::create_dir_all(&config_dir).unwrap();
        std::fs::write(&path, "opened document").unwrap();
        std::env::set_var("AITEXT_CONFIG_DIR", config_dir.join("config"));
        ...
        assert_eq!(app.workspace.current().unwrap().text(), "opened document");
