...
impl AitextApp {
    ...
    pub fn select_document(&mut self, id: u64) -> bool {
        if self.workspace.current_id() == Some(id) {
            return true;
        }
        if self.workspace.get(id).is_none() {
            return false;
        }
        self.cancel_completion();
        let selected = self.workspace.set_current(id);
        if selected {
    ...
    pub fn close_document(&mut self, id: u64) -> bool {
        let was_current = self.workspace.current_id() == Some(id);
        ...
        let closed = self.workspace.close(id).is_some();
        if closed && was_current && self.workspace.current().is_some() {
            self.queue_completion();
        }
        closed
    }
    ...
    pub fn dispatch(&mut self, command: Command) {
        match command {
            ...
            Command::NextTab => {
                let old_id = self.workspace.current_id();
                self.workspace.next_tab();
                if self.workspace.current_id() != old_id {
                    self.invalidate_and_queue();
                }
            }
            Command::PrevTab => {
...
mod tests {
    ...
    #[test]
    fn creating_a_new_tab_invalidates_old_completion_results() {
        let mut app = AitextApp::new_for_test();
        app.dispatch(Command::NewTab);
        ...
        app.completion
            .engine
            .on_result(generation, Ok(" old result".into()));

        assert_eq!(
            app.workspace.current().unwrap().display_name(),
            "Untitled-2"
        );
    ...
    #[test]
    fn closing_a_tab_invalidates_late_completion_results() {
        let mut app = AitextApp::new_for_test();
        app.dispatch(Command::NewTab);
        app.completion.engine.configured = true;
        let snapshot = aitext_ai::CompletionSnapshot {
    ...
    #[test]
    fn closing_one_of_two_tabs_queues_completion_for_the_new_active_tab() {
        let mut app = AitextApp::new_for_test();
        app.dispatch(Command::NewTab);
        app.workspace.current_mut().unwrap().insert("first");
        app.dispatch(Command::NewTab);
        app.workspace.current_mut().unwrap().insert("second");
        let mut profile =
        ...
        let old_generation = match app.completion.engine.on_tick(60) {
            aitext_ai::EngineEvent::StartRequest { snapshot } => snapshot.generation,
            _ => panic!("expected a completion request"),
        };
        ...
        assert_eq!(app.workspace.current().unwrap().text(), "first");
        assert!(app.completion.engine.has_pending());
        assert_ne!(app.completion.engine.generation(), old_generation);
    }

    #[test]
    fn switching_tabs_invalidates_old_completion_and_queues_the_new_document() {
        let mut app = AitextApp::new_for_test();
        app.dispatch(Command::NewTab);
        app.workspace.current_mut().unwrap().insert("first");
        app.dispatch(Command::NewTab);
        ...
        app.completion
            .engine
            .on_result(old_generation, Ok(" old result".into()));
    ...
    #[test]
    fn selecting_an_explicit_tab_invalidates_old_completion_results() {
        let mut app = AitextApp::new_for_test();
        app.dispatch(Command::NewTab);
        let first_id = app.workspace.current_id().unwrap();
        app.workspace.current_mut().unwrap().insert("first");
        app.dispatch(Command::NewTab);
        app.workspace.current_mut().unwrap().insert("second");
        app.completion.engine.configured = true;
        ...
        assert!(app.select_document(first_id));
        app.completion
            .engine
            .on_result(generation, Ok(" old result".into()));

        assert_eq!(app.workspace.current_id(), Some(first_id));
        assert!(app.completion.engine.suggestion().is_none());
        assert_ne!(app.completion.engine.generation(), generation);
    }
    ...
    #[test]
    fn ime_preedit_invalidates_a_completion_already_in_flight() {
        let mut app = AitextApp::new_for_test();
        app.dispatch(Command::NewTab);
        app.completion.engine.configured = true;
        let snapshot = aitext_ai::CompletionSnapshot {
        ...
        assert!(app.completion.composing);
        assert!(app.completion.engine.suggestion().is_none());
        assert_ne!(app.completion.engine.generation(), generation);
    }
