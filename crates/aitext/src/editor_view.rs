...
pub fn draw_editor(ui: &mut Ui, app: &mut AitextApp) {
    ...
    let completion_prefix = app
        .workspace
        .current()
        .map(|doc| aitext_ai::take_snapshot(doc, 0).prefix)
        .unwrap_or_default();
    let ghost = app
        .ghost_text()
        .map(|text| aitext_ai::repair_unclosed_code_completion(&completion_prefix, text));
...
fn draw_editor_context_menu(ui: &mut Ui, app: &mut AitextApp) {
...
mod tests {
    ...
    fn settings_text_input_does_not_edit_the_document() {
        ...
        let mut input = test_raw_input(400.0, 220.0);
    ...
    fn copy_event_writes_selected_text_to_the_system_clipboard_output() {
        ...
        document.set_selection(aitext_core::Selection {
            anchor: 0,
            caret: 5,
        });
    ...
    fn editor_display_repairs_an_incomplete_code_ghost() {
        ...
        let context = egui::Context::default();
        let input = test_raw_input(500.0, 220.0);
        let output = context.run(input, |context| {
            egui::CentralPanel::default()
                .frame(egui::Frame::NONE.inner_margin(egui::Margin::ZERO))
                .show(context, |ui| draw_editor(ui, &mut app));
    ...
    fn right_click_opens_the_editor_context_menu() {
        ...
        let initial = test_raw_input(400.0, 220.0);
        ...
        let mut press = test_raw_input(400.0, 220.0);
        ...
        let mut release = test_raw_input(400.0, 220.0);
        ...
        let mut visible = test_raw_input(400.0, 220.0);
