...
pub fn ghost_mode(snapshot: &CompletionSnapshot) -> GhostMode {
    if looks_like_code(&snapshot.prefix) {
...
pub fn request_body(snapshot: &CompletionSnapshot, model: &str) -> serde_json::Value {
    request_body_for(CompletionProtocol::OpenAiChat, snapshot, model)
}
...
pub fn request_body_for(
    ...
    match protocol {
        ...
        CompletionProtocol::OpenAiChat => chat_body(snapshot, model),
        CompletionProtocol::OpenAiResponses => responses_body(snapshot, model),
        CompletionProtocol::AnthropicMessages => anthropic_messages_body(snapshot, model),
    }
}
...
fn openai_chat_body(snapshot: &CompletionSnapshot, model: &str) -> serde_json::Value {
    let mut body = chat_body(snapshot, model);
...
pub fn completion_request_plan(
    ...
    CompletionRequestPlan {
        ...
        auth,
        headers,
        body: if protocol == CompletionProtocol::OpenAiChat
            && config.provider == ProviderKind::OpenAi
        {
            openai_chat_body(snapshot, &config.model)
        } else {
            request_body_for(protocol, snapshot, &config.model)
        },
    }
}
