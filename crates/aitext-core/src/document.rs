...
impl Document {
    ...
    pub fn text_window_around_caret(
        &self,
        prefix_chars: usize,
        suffix_chars: usize,
    ) -> (String, String) {
        let caret = self.selection.caret.min(self.rope.len_chars());
        let prefix_start = caret.saturating_sub(prefix_chars);
        let suffix_end = caret
            .saturating_add(suffix_chars)
            .min(self.rope.len_chars());
        (
            self.rope.slice(prefix_start..caret).to_string(),
            self.rope.slice(caret..suffix_end).to_string(),
        )
    }
...
mod tests {
    ...
    #[test]
    fn text_window_around_caret_returns_only_the_requested_context() {
        let mut doc = Document::from_text("0123456789abcdefghij");
        doc.set_caret(10);

        assert_eq!(
            doc.text_window_around_caret(4, 5),
            ("6789".to_string(), "abcde".to_string())
        );
    }

    #[test]
    fn text_window_clamps_extreme_context_sizes_without_overflowing() {
        let mut doc = Document::from_text("hello");
        doc.set_caret(2);

        assert_eq!(
            doc.text_window_around_caret(usize::MAX, usize::MAX),
